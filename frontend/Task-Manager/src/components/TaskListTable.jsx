  ...
  return (
    ...
    <table className="min-w-full">
        ...
        {tableData.map((task) => (
            ...
            <td className="my-3 mx-4 text-gray-700 text-[13px] line-clamp-1 overflow-hiddden">{task.title}</td>
            <td className="py-4 px-4">
                <span className={`px-2 py-1 text-xs rounded text-xs inline-block ${getStatusBadgeColor(task.status)}`}>{task.status}</span>
            </td>
            <td className="py-4 px-4">
                <span className={`px-2 py-1 text-xs rounded text-xs inline-block ${getPriorityBadgeColor(task.priority)}`}>{task.priority}</span>
            </td>
